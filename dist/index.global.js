...
(() => {
  ...
  (function(Scratch2) {
    ...
    class OmegaRTC {
       ...
       * @param {boolean} setup - Optional: If the connection is used for voice, set to true if we want to transmit audio. Otherwise, only accept incoming audio.
       */
      async handleAnswer(id, mode, answer, setup) {
        try {
          const conn = await this.getConnectionObject(
            id,
            mode,
       ...
       * @param {boolean} setup - Optional: If the connection is used for voice, set to true if we want to transmit audio. Otherwise, only accept incoming audio.
       */
      async handleIceCandidate(id, mode, candidate, setup) {
        try {
          const conn = await this.getConnectionObject(
            id,
            mode,
    ...
    class ScratchWebRTC {
      ...
      constructor(vm) {
        this.vm = vm;
        this.dataOffers = /* @__PURE__ */ new Map();
        this.voiceOffers = /* @__PURE__ */ new Map();
        this.dataAnswers = /* @__PURE__ */ new Map();
        this.voiceAnswers = /* @__PURE__ */ new Map();
      ...
      getOffer(args) {
        switch (args.mode) {
          case "data":
            return this.dataOffers.has(args.peer) ? btoa(JSON.stringify(this.dataOffers.get(args.peer))) : "";
          case "voice":
            return this.voiceOffers.has(args.peer) ? btoa(JSON.stringify(this.voiceOffers.get(args.peer))) : "";
          default:
            return "";
        }
      }
      async createOffer(args) {
        ...
        switch (args.mode) {
          case "data":
            ...
            if (offer)
              this.dataOffers.set(args.peer, offer);
            break;
          case "voice":
            offer = await webrtc.createOffer(args.peer, args.peer, 1, true);
            if (offer)
              this.voiceOffers.set(args.peer, offer);
            break;
        }
      }
      getAnswer(args) {
        switch (args.mode) {
          case "data":
            return this.dataAnswers.has(args.peer) ? btoa(JSON.stringify(this.dataAnswers.get(args.peer))) : "";
          case "voice":
            return this.voiceAnswers.has(args.peer) ? btoa(JSON.stringify(this.voiceAnswers.get(args.peer))) : "";
          default:
            return "";
        }
      }
      async createAnswer(args) {
        ...
        switch (args.mode) {
          case "data":
            ...
            if (answer)
              this.dataAnswers.set(args.peer, answer);
            break;
          case "voice":
            answer = await webrtc.createAnswer(
              args.peer,
            ...
            if (answer)
              this.voiceAnswers.set(args.peer, answer);
            break;
        }
      }
