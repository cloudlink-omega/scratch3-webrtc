  ...
  (function (Scratch2) {
    ...
    class OmegaRTC {
      ...
      async createOffer(id, name, mode, setup) {
        try {
          const conn = await this.getConnectionObject(id, mode, setup);
          conn.name = name;
      ...
      async handleAnswer(id, mode, answer) {
        try {
          const conn = await this.getConnectionObject(id, mode);
      ...
      getDataConnection(id) {
        if (this.dataConnections.has(id)) return this.dataConnections.get(id);
      ...
      async getVoiceConnection(id, setup) {
        if (this.voiceConnections.has(id)) return this.voiceConnections.get(id);
        const conn = new PeerConnection(this.configuration);
